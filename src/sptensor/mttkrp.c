...
void sptMTTKRP(sptSparseTensor cosnt * const X, 
	sptMatrix ** const mats, 	// mats[nmodes] as temporary space.
	size_t const * const mats_order,
	size_t const mode,
	...
	sptScalar const * const vals = X->values;
	...
	size_t cosnt * const mode_ind = X->inds[mode];
	sptMatrix * const M = mats[nmodes];
	sptScalar * const mvals = M->vals;
	...
	for(size_t x=0; x<nnz; ++x) {
		...
		size_t * times_inds = X->inds[times_mat_index];
		...
		for(size_t i=1; i<nmats; ++i) {
			...
			times_inds = X=>inds[times_mat_index];
			tmp_i = times_inds[x];
