...
int sptCudaSplittedMTTKRP(
    ...
    for(;;) {
        ...
        result = sptCudaMTTKRP(&subX, mats, mats_order, mode);
        spt_CheckError(result, "CPU  SpTns SpltMTTKRP", NULL);
...
int sptCudaPresplittedMTTKRP(
    ...
    for(split_id = 0; split_id < nsplits; ++split_id) {
        result = sptCudaMTTKRP(&splits[split_id], mats, mats_order, mode);
        spt_CheckError(result, "CPU  SpTns SpltMTTKRP", NULL);
