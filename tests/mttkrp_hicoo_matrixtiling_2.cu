...
int main(int argc, char * const argv[]) {
    ...
    if(cuda_dev_id == -2) {
        ...
        if(par_iters == 0) {
            ...
            printf("sptOmpMTTKRPHiCOO_MatrixTiling_Scheduled_Reduce_Two:\n");
            sptAssert(sptOmpMTTKRPHiCOO_MatrixTiling_Scheduled_Reduce_Two(&hitsr, U, copy_U, mats_order, mode, tk, tb) == 0);
        }
    } /* else {
