...
declare -a test_tsr_names=("flickr" "freebase_sampled" "nell1" "delicious")
declare -a threads=("1")
...
do
	...
	do
		...
		sb=7
		renum=1
		echo "./build/tests/mttkrp_hicoo_renumber -i ${tsr_path}/${tsr_name}_nnz.tns -b ${sb} -k ${sk} -c ${sc} -d ${dev_id} -r ${R} -e ${renum} > ${out_path}/${tsr_name}-b${sb}-k${sk}-c${sc}-r${R}-e${renum}-renumber-seq.txt"
		./build/tests/mttkrp_hicoo_renumber -i ${tsr_path}/${tsr_name}_nnz.tns -b ${sb} -k ${sk} -c ${sc} -d ${dev_id} -r ${R} -e ${renum} > ${out_path}/${tsr_name}-b${sb}-k${sk}-c${sc}-r${R}-e${renum}-renumber-seq.txt
