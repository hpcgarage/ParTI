...
declare -a test_tsr_names=("choa700k" "1998DARPA" "freebase_music" "flickr")
...
do
	...
	do
		...
		# dev_id=-2
		# echo "./build/tests/mttkrp_renumber_newsort -i ${tsr_path}/${tsr_name}_nnz.tns -d ${dev_id} -r ${R} -e ${renum} -n ${niters_renum} > ${out_path}/${tsr_name}-r${R}-e${renum}-n${niters_renum}-seq.txt"
		# ./build/tests/mttkrp_renumber_newsort -i ${tsr_path}/${tsr_name}_nnz.tns -d ${dev_id} -r ${R} -e ${renum} -n ${niters_renum} > ${out_path}/${tsr_name}-r${R}-e${renum}-n${niters_renum}-seq.txt
		...
		for tk in ${threads[@]}
		do
			echo "numactl --interleave=0-1 ./build/tests/mttkrp_renumber_newsort -i ${tsr_path}/${tsr_name}_nnz.tns -d ${dev_id} -r ${R} -t ${tk} -e ${renum} -n ${niters_renum} -u ${use_reduce} > ${out_path}/${tsr_name}-r${R}-tk${tk}-e${renum}-n${niters_renum}-reduce.txt"
			numactl --interleave=0-1 ./build/tests/mttkrp_renumber_newsort -i ${tsr_path}/${tsr_name}_nnz.tns -d ${dev_id} -r ${R} -t ${tk} -e ${renum} -n ${niters_renum} -u ${use_reduce} > ${out_path}/${tsr_name}-r${R}-tk${tk}-e${renum}-n${niters_renum}-reduce.txt
