...
out_path="/global/homes/j/jiajiali/Work/ParTI-dev/timing-results/parti/coo/single-renumber-bfs"
...
do
	...
	do
		...
		# dev_id=-2
		# renum=1
		# echo "./build/tests/mttkrp_renumber -i ${tsr_path}/${tsr_name}.tns -d ${dev_id} -r ${R} -e ${renum} > ${out_path}/${tsr_name}-r${R}-e${renum}-renumber-seq.txt"
		# ./build/tests/mttkrp_renumber -i ${tsr_path}/${tsr_name}.tns -d ${dev_id} -r ${R} -e ${renum} > ${out_path}/${tsr_name}-r${R}-e${renum}-renumber-seq.txt
		...
		for tk in ${threads[@]}
		do
			echo "numactl --interleave=0-1 ./build/tests/mttkrp_renumber -i ${tsr_path}/${tsr_name}_nnz.tns -d ${dev_id} -r ${R} -t ${tk} -e ${renum} > ${out_path}/${tsr_name}-r${R}-tk${tk}-e${renum}-renumber.txt"
			numactl --interleave=0-1 ./build/tests/mttkrp_renumber -i ${tsr_path}/${tsr_name}_nnz.tns -d ${dev_id} -r ${R} -t ${tk} -e ${renum} > ${out_path}/${tsr_name}-r${R}-tk${tk}-e${renum}-renumber.txt
		done
