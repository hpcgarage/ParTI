...
do
	...
	do
		...
		do
			...
			echo "${splatt_path}/bin/splatt cpd ${tsr_path}/${tsr_name}.tns --csf=all -r ${R} -t ${th} --nowrite > ${out_path}/${tsr_name}-r${R}-t${th}.txt"
			${splatt_path}/bin/splatt cpd ${tsr_path}/${tsr_name}.tns --csf=all -r ${R} -t ${th} --nowrite > ${out_path}/${tsr_name}-r${R}-t${th}.txt
