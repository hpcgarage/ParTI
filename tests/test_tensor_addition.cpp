...
int main(int argc, char const* argv[]) {
	...
	CFile fY(args[1], "r");
	SparseTensor Y = SparseTensor::load(fY, 1);
	fY.fclose();

	std::printf("Y = %s\n", Y.to_string(!dense_format, limit).c_str());
	Timer timer(cpu);
	timer.start();
	SparseTensor Z = tensor_addition(X, Y);
	timer.stop();
 	std::printf("Z = %s\n", Z.to_string(!dense_format, limit).c_str());
	timer.print_elapsed_time("tensor addition time");
	if (args.size() == 3) {
		CFile fZ(args[2], "w");
		Z.dump(fZ, 1);
	}
